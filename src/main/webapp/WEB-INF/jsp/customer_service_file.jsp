...
function openNewTab(url,icons) {
	...
	$(function(){
		...
		/*展示数据的datagrid表格*/
		$("#datagrid").datagrid({
			url:'${ctx}/customerServiceFile/findAll.action?status=已处理',
			method:'get',
			fit:true,
			singleSelect:false,
			toolbar:'#toolbar',
	...
	function doSave() {
		$("#status").val("已归档");
		$('#form').form('submit', {    
		    url:"${ctx}/customerServiceFile/update.action",    
		    onSubmit: function(){    
	...
	function doDelete(){
		...
		$.messager.confirm("系统提示", "您确认要删除么", function(r){
			if (r){
				$.post(
					"${ctx}/customerServiceFile/delete.action",
					{ids:ids}, 
					function(result) {
	...
	function openAddDialog() {
		...
		url = "${ctx}/customerServiceFile/add.action";
		$('#form').form("clear");
		...
		<form action="" id="form" method="post">
			...
			<table cellspacing="8px">
		   		...
		   		<tr>
		   			<td>分配给：</td>
		   			<td>
		   				<input class="easyui-combobox" readonly="readonly" id="assigner" name="assigner" data-options="panelHeight:'auto',editable:false,valueField:'trueName',textField:'trueName',url:'${ctx}/customerServiceFile/findAssignMan.action'"/>
		   			</td>
		   			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		   			<td>分配时间：</td>
	   			...
	   			<tr>
		   			<td>处理人：</td>
		   			<td>
		   				<input class="easyui-combobox" id="serviceDealPeople" name="serviceDealPeople" data-options="panelHeight:'auto',editable:false,valueField:'trueName',textField:'trueName',url:'${ctx}/customerServiceFile/finddisposeMan.action'"/>
		   			</td>
		   			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		   			<td>处理时间：</td>
