...
function openNewTab(url,icons) {
	...
	$(function(){
		...
		/*展示数据的datagrid表格*/
		$("#datagrid").datagrid({
			url:'${ctx}/customerServiceFeedback/findAll.action?status=已处理',
			method:'get',
			fit:true,
			singleSelect:false,
			toolbar:'#toolbar',
	...
	function doSave() {
		$("#status").val("已归档");
		$('#form').form('submit', {    
		    url:"${ctx}/customerServiceFeedback/update.action",    
		    onSubmit: function(){    
	...
	function doDelete(){
		...
		$.messager.confirm("系统提示", "您确认要删除么", function(r){
			if (r){
				$.post(
					"${ctx}/customerServiceFeedback/delete.action",
					{ids:ids}, 
					function(result) {
	...
	function openAddDialog() {
		...
		url = "${ctx}/customerServiceFeedback/add.action";
		$('#form').form("clear");
...
<div id="dialog" class="easyui-dialog" closed="true" modal="true"
		...
		<form action="" id="form" method="post">
			...
			<input type="hidden" id="status" name="status" value="已归档"/>
			<table cellspacing="8px">
		   		...
		   		<tr>
		   			<td>分配给：</td>
		   			<td>
		   				<input class="easyui-combobox" readonly="readonly" id="assigner" name="assigner" data-options="panelHeight:'auto',editable:false,valueField:'trueName',textField:'trueName',url:'${ctx}/customerServiceFeedback/findAssignMan.action'"/>
		   			</td>
		   			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		   			<td>分配时间：</td>
	   			...
	   			<tr>
		   			<td>处理人：</td>
		   			<td>
		   				<input class="easyui-combobox" id="serviceDealPeople" name="serviceDealPeople" data-options="panelHeight:'auto',editable:false,valueField:'trueName',textField:'trueName',url:'${ctx}/customerServiceFeedback/finddisposeMan.action'"/>
		   			</td>
		   			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		   			<td>处理时间：</td>
